...
def gros_laser() :
	for i in range(4):
		x,y=extract_data('0B_700uW_7E6coups'+'.txt',xcol=2*i,ycol=2*i+1)
		y_avg=sum(y[0:99])/100
		y=y/y_avg
		plt.plot(x,y)
	plt.show()
...
def petit_laser() :
	for i in range(15):
		x,y=extract_data('0B_18uW_7E6coups'+'.txt',xcol=2*i,ycol=2*i+1)
		y_avg=sum(y[0:99])/100
		y=y/y_avg
		plt.plot(x,y)
	plt.show()
...
def moyen_laser():
	for i in range(15):
		x,y=extract_data('0B_80uW_7E6coups'+'.txt',xcol=2*i,ycol=2*i+1)
		y_avg=sum(y[0:99])/100
		y=y/y_avg
		plt.plot(x,y)
	plt.show()
