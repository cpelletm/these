...
x,y=extract_data('x=%.6f,y=%.6f'%(xs[0],xs[0]),xcol=2,ycol=3)
y=y/sum(y)
plt.plot(y,'x-')

x,y=extract_data('x=%.6f,y=%.6f'%(xs[0],xs[5]),xcol=2,ycol=3)
y=y/sum(y)
plt.plot(y,'x-')

x,y=extract_data('x=%.6f,y=%.6f'%(xs[0],xs[10]),xcol=2,ycol=3)
y=y/sum(y)
plt.plot(y,'x-')
