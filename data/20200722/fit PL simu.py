...
def PL_100(B,gamma_las=1E-2,gamma_t1=3E-4):
	Sz=np.array([[1,0,0],[0,0,0],[0,0,-1]])
	Sx=1/np.sqrt(2)*np.array([[0,1,0],[1,0,1],[0,1,0]])
	Sy=1/(np.sqrt(2)*1j)*np.array([[0,1,0],[-1,0,1],[0,-1,0]])
	...
	def make_collapse_list(gamma_las,gamma_t1) :
		...
		for i in range(3) :
			for j in range(3) :
				if i!=j :
					single=np.zeros((3,3))
					single[i,j]=np.sqrt(gamma_t1)
					col+=[Qobj(single)]
		las=np.zeros((3,3))
		las[1,0]=np.sqrt(gamma_las)
		col+=[Qobj(las)]
		las=np.zeros((3,3))
		las[1,2]=np.sqrt(gamma_las)
		col+=[Qobj(las)]
		return col
...
norm_factor=PL_100(0)
y_simu=[PL_100(B)/norm_factor for B in x]
plt.plot(x,y)
plt.plot(x,y_simu)
plt.show()