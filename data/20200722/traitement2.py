...
def fit_ordre_5(x,y) :
	...
	z,a,b,c,d,e = np.linalg.lstsq(A, y, rcond=None)[0]
	return([z,a,b,c,d,e],z*x**5+a*x**4+b*x**3+c*x**2+d*x+e)
...
def fit_ordre_6_CI(x,y,a,b,c,d,e) :
	p0=[0,0,a,b,c,d,e]
	def f(x,w,z,a,b,c,d,e) :
		return w*x**6+z*x**5+a*x**4+b*x**3+c*x**2+d*x+e
...
def ESR_n_pics_freq_fixe(x,y,cs,width=8,ss=None,amp=None) :
	if not ss :
		ss=y[0]
	if not amp :
		if max(y)-ss > ss-min(y) :
...
fname='scan_100_rose_10V'+'.txt'
x0=507
x,ybrut=extract_data(fname)
ybrut=ybrut/max(ybrut)
# plt.plot(ybrut[x0:])
...
# y=np.flip(ybrut[:x0])
# x=np.arange(len(y))*0.33
# [a,b,c,d,e],yfit=fit_ordre_4(x[xmin:xmax],y[xmin:xmax])
# yfit=a*x**4+b*x**3+c*x**2+d*x+e

# plt.plot(x[xmin:xmax],y[xmin:xmax]-yfit[xmin:xmax])


y=ybrut[x0:] 
x=np.arange(len(y))*0.33

with open('scan 100 calibre.txt','w') as f:
...
y=list(y)
x=list(x)
y_to_fit=np.array(y[28:42]+y[91:148]+y[183:353]+y[401:-1])
x_to_fit=np.array(x[28:42]+x[91:148]+x[183:353]+x[401:-1])
y=np.array(y)
x=np.array(x)
[a,b,c,d,e],yfit=fit_ordre_4(x_to_fit,y_to_fit)
yfit=a*x**4+b*x**3+c*x**2+d*x+e

print(a,b,c,d,e)

# plt.plot(x[xmin:xmax],y[xmin:xmax],lw=2.5)
# plt.plot(x[xmin:xmax],yfit[xmin:xmax],ls='dashed',lw=2)
...
for x_v in x_C13 :
	plt.plot([x_v,x_v],[1,-1],'--',color=c2)
# popt,yfit=ESR_n_pics_freq_fixe(x[:70],y[:70],x_C13,width=1)
# plt.plot(x[:70],yfit)
