...
def plot_champs_transverses():
	...
	# x,y=extract_data(fname,ycol=5)
	# y=y/max(y)
	# plt.plot(x,y,label='moyen champ transverse',marker='o',markerfacecolor="None",ms=5,mew=1)
	...
	# x,y=extract_data(fname,ycol=5)
	# y=y/max(y)
	# plt.plot(x,y,label='Baseline 2',marker='o',markerfacecolor="None",ms=5,mew=1)
	plt.xticks(fontsize=15)
	plt.yticks(fontsize=15)
	plt.legend()
	plt.show()
...
def fig121_Vs_22():
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'o',markerfacecolor="None",color='tab:blue',ms=5,mew=1)
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'s',markerfacecolor="None",color='tab:blue',ms=5,mew=1)
	fname='T1 2x2'
	x,y=extract_data(fname,ycol=5)
	nmax=len(x)//4
	x=x[:nmax]
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'^',markerfacecolor="None",color='tab:blue',ms=5,mew=1)
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'o',color='tab:orange',ms=5,mew=1)
	fname='T1 1x2x1'
	x,y=extract_data(fname,ycol=5)
	nmax=len(x)//4
	x=x[:nmax]
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'s',color='tab:orange',ms=5,mew=1)
	fname='T1 121 (3)'
	x,y=extract_data(fname,ycol=5)
	nmax=len(x)//4
	x=x[:nmax]
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'^',color='tab:orange',ms=5,mew=1)
	fname='T1 121 (4)'
	x,y=extract_data(fname,ycol=5)
	nmax=len(x)//4
	x=x[:nmax]
...
def find_T1_for_all_oreintation():
