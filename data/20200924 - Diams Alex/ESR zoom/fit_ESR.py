...
def ESR_n_pics(x,y,cs,width=8,ss=None,amp=None) :
	...
	for p in popt :
		variables+=(p,)
	n=(len(variables)-2)//3
	popt=[variables[2:2+n]]+[variables[2+n:2+2*n]] #retourne [[centres],[largeurs]]
...
x,y=extract_data('N2O 250 ppm +10dBm.txt')
x=x*1000
y=y/max(y)
plt.plot(x,y,'x',color=color)
popt,yfit=ESR_n_pics(x,y,[2742,2744,2747],width=1) #Attention, pics en MHz
largeurs=popt[1]
gamma_avg=sum(largeurs)/3
plt.plot(x,yfit,lw=2,color=color,label='width=%f MHz'%gamma_avg)
