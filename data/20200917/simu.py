...
x,y=extract_data('scan_adamas_121.txt')
n=len(x)
nmax=400
fig,ax=plt.subplots(2)

B0=[100.80643063 ,79.56200891, 16.52542669]
B0_cart=spher_to_cart(B0[1]*np.pi/180,B0[2]*np.pi/180)*B0[0]
B1=[104.44675241,  90.          ,15.38019421]
B1_cart=spher_to_cart(B1[1]*np.pi/180,B1[2]*np.pi/180)*B1[0]
B2=[98.94514891,72.27479592,18.64027679]
B2_cart=spher_to_cart(B2[1]*np.pi/180,B2[2]*np.pi/180)*B2[0]

Bmin=B0_cart+(B1_cart-B0_cart)*2.34/0.75
Bmax=B0_cart+(B2_cart-B0_cart)*2.34/0.55
B_in=Bmin
B_out=Bmax
...
x=np.linspace(0,np.linalg.norm(B_out-B_in),n)
y=y/max(y)
ax[0].plot(x[:nmax],y[:nmax])
...
for j in range(4):
	ax[1].plot(x[:nmax],transis[:nmax,j],color=color,ls='--')
...
ax[1].set_xlabel(r'$B_{em}$(G)',fontsize=22)
ax[0].set_ylabel(r'Photoluminescence',fontsize=19)
ax[1].set_ylabel(r'Frequency (MHz)',fontsize=19)
