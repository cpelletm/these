...
def err_func(B_coord,ESR_peaks) :
	...
	err=np.linalg.norm(ESR_B-ESR_peaks)
	return(err)

def err_func_minus(B_coord,ESR_peaks) :
	ESR_peaks=np.sort(ESR_peaks)
	B=mag_field(B_coord[0],B_coord[1],B_coord[2],radian=False)
...
def find_B(ESR_peaks):
	if len(ESR_peaks)==8:
		sol=minimize(err_func,x0=[100,72.3,22.5],args=ESR_peaks,bounds=[(0,1000),(54.74,90),(0,45)]) #Ca fait 1/16e d'espace, il faudrait 1/48e pour être bijectif mais les expressions deviennent moches
	if len(ESR_peaks)==4:
		sol=minimize(err_func_minus,x0=[100,72.3,22.5],args=ESR_peaks,bounds=[(0,1000),(54.74,90),(0,45)])
...
def check_peaks():
	fname='ESR_0V'
	ESR=[2656,2727,2764,2829,2980,3030,3058,3106]
...
ESR_0V=[2656,2727,2764,2829,2980,3030,3058,3106]
ESR_0V_minus=[2656,2727,2764,2829]

ESR_moins_0_66V=[2685,2685,2789,2789]
ESR_0_8V=[2638,2755,2755,2860]
# ESR_04V=[2570,2738,2759,2898,2955,3068,3087,3188]

# B=mag_field(B[0],B[1],B[2],radian=False)
# print(transitions(B))
ESR=ESR_0V_minus
B=find_B(ESR)
print(B)
print(err_func_minus(B,ESR))
