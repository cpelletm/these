...
def ESR_n_pics(x,y,cs,width=8,ss=None,amp=None,typ='gauss') : #typ="gauss" ou "lor"
	if not ss :
		ss=y[0]
	if not amp :
		if max(y)-ss > ss-min(y) :
...
x,y=extract_data('ESR 1 raie tranche objectif bleu 500 uW 5 dBm')
x=x*1e3
centres=[2996.5,2999,3001]

plt.plot(x,y,'o',markerfacecolor="None",ms=5,mew=1)


popt,yfit=ESR_n_pics(x,y,centres,width=1,typ="gauss")
width=sum(popt[4:7])/3
plt.plot(x,yfit,label="width=%4.3f MHz"%width)
