...
def ask_name():
	...
	fname,filters=QFileDialog.getOpenFileName(caption="Chose a data file",directory="./",filter="Data files (*.txt)")	
	return fname
...
x,y=extract_data('3V.txt')
plt.plot(x,y,'x',label='experimental')

popt,yfit=exp_fit(x,y)
tau=popt[2]
plt.plot(x,yfit,label='exp fit, tau=%E'%tau)

popt,yfit=stretch_exp_fit(x,y)
tau=popt[2]
plt.plot(x,yfit,label='stretch exp fit, tau=%E'%tau)
