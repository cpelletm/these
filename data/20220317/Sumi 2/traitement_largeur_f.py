...
def full_animation():
	...
	fnames,fval=extract_glob('Série ESR',14)
	...
	fnames,fval=extract_glob('Série T1',13)
	fnames=fnames[nFrameMin:nFrameMax]
	...
	popt,yfit=stretch_arb_exp_fit_zero(x,y,alpha=1,fixed=True)
	...
	for i in range(len(fnames)):
		...
		y=y[:nT1max]/max(y[:nT1max])
		yT1s+=[y]
		popt,yfit=stretch_arb_exp_fit_zero(xT1,y,alpha=1,fixed=True)
		yfits+=[yfit]
		taus+=[popt[1]]

	T1=taus[0]
	t=ax2.text(x=4,y=0.5,s=r'$T_1$=%.3f ms'%T1,fontsize=20,fontweight='bold')
