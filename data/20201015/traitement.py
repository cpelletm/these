...
def ecris_gros():
	plt.figure(num=1,figsize=(8,6),dpi=80) #à écrire au début a priori


	ax=plt.gca()
	ax.ticks_param(fontsize=20)
...
x_cal=[6.85,5.77,2.03]
y_cal=[510,592,1020]
[B0,x0],y_cal=fit_B_dipole(x_cal,y_cal)


plt.figure(num=1,figsize=(9,6),dpi=80)
xmax=250
x,y=extract_data('scan_PL.txt')
y=y/max(y)
x_B=B0/(x+x0)**3
plt.plot(x_B[:xmax],y[:xmax],'-',linewidth=3,ms=8,mew=2)
ax=plt.gca()
