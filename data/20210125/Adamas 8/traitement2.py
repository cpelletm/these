...
def ESR_n_pics(x,y,cs,width=8,ss=None,amp=None,peak_form='Gauss') :
	if not ss :
		ss=y[0]
	if not amp :
		if max(y)-ss > ss-min(y) :
	...
	if type(width) is np.float64 or type(width) is float :
		widths=np.ones(n)*width
	else :
		widths=np.array(width)
	if type(amp) is np.float64 or type(amp) is float :
		amps=np.ones(n)*amp
	else :
		amps=np.array(amp)
	p0=[ss]
...
# xmin=4
# xmax=60
# x,y=extract_data('T1_1x1x1x1.txt')
# x=x[xmin:]
# y=y[xmin:]
# y=y/max(y)
...
xmin=0
xmax=-1
x,y=extract_data('ESR_0B_-15dB_1.4mW.txt')
x=x[xmin:xmax]
y=y[xmin:xmax]
plt.plot(x,y,'o',markerfacecolor="None")
popt,yfit=ESR_n_pics(x,y,[2.865,2.865],width=[0.01,0.003],amp=[-0.004,0.04],peak_form='Gauss')
print(popt)
plt.plot(x,yfit,label='nu1=%4.3f,nu2=%4.3f,Deltanu=%3.2f MHz'%(popt[1],popt[2],(popt[2]-popt[1])*1e3))

# xmin=0
# xmax=-1
# x,y=extract_data('ESR_1_pic_zoom.txt')
# x=x[xmin:xmax]
# y=y[xmin:xmax]
# plt.plot(x,y,'o',markerfacecolor="None")
# popt,yfit=ESR_n_pics(x,y,[2.57],width=0.02)
# print(popt)
# plt.plot(x,yfit)
