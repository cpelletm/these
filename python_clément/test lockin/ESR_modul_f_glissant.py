...
class Photon_Counter(QMainWindow):
	def __init__(self):
		...
		self.sampling_rate=300 #Ne pas monter au dessus de 1E3
		self.n_points=301
		self.f_min=2.8
		self.f_max=2.95 #GHz      
		self.level=10 #dBm
		...
		self.labelfreq_dev=QLabel("freq_dev")
		self.lectfreq_dev=QLineEdit('%3.2E'%self.freq_dev)
		Vbox_gauche.addWidget(self.labelfreq_dev)
		Vbox_gauche.addWidget(self.lectfreq_dev)
	...
	def update_value(self):
		self.sampling_rate=np.float(self.lectsampling_rate.text())
		self.freq_dev=np.float(self.lectfreq_dev.text())
		...
		self.n_acq=self.n_points*self.n_glissant
	...
	def start_measure(self):
		...
		self.tension.timing.cfg_samp_clk_timing(self.sampling_rate*self.n_glissant,sample_mode=nidaqmx.constants.AcquisitionType.CONTINUOUS, samps_per_chan=self.n_acq)

		self.trig_uW=nidaqmx.Task()
		self.trig_uW.do_channels.add_do_chan('Dev1/port0/line1')
		self.trig_uW.timing.cfg_samp_clk_timing(self.sampling_rate*(self.n_glissant+1),sample_mode=nidaqmx.constants.AcquisitionType.CONTINUOUS, samps_per_chan=self.n_acq)

		
		#self.trig_uW.triggers.start_trigger.cfg_dig_edge_start_trig('/Dev1/Ctr1Source')  


		signal=([True]+[False]*(self.n_glissant))*self.n_points
	...
	def take_point(self):
		...
		PL_brut=np.array(self.tension.read(self.n_acq,timeout=nidaqmx.constants.WAIT_INFINITELY))
	...
	def config_uW(self):
		...
		self.PG.write(':SOUR:FM:DEV %e'%self.freq_dev)
		self.PG.write('*WAI')
