...
class Photon_Counter(QMainWindow):
	def __init__(self):
		...
		self.trace_ax,self.dynamic_ax = dynamic_canvas.figure.subplots(2)

		self.x=np.zeros(100)
		self.y=np.zeros(100)
		self.dynamic_line,=self.dynamic_ax.plot(self.x,self.y,'x')
		self.fit_line,=self.dynamic_ax.plot(self.x,self.y,label='fit')
		self.dynamic_line_2,=self.dynamic_ax.plot(self.x,self.y,'x')
		self.fit_line_2,=self.dynamic_ax.plot(self.x,self.y,label='fit')
		...
		self.trace_line,=self.trace_ax.plot(self.x,self.y,label='Sensi=0')
	...
	def update_value(self):
		...
		self.t=np.linspace(0,self.n_acq*self.dt_meas,self.n_acq)
	...
	def take_point(self):
		...
		self.trace_line.set_data(self.t,lecture_brut)
		xmin=min(self.t)
		xmax=max(self.t)
		ymin=min(lecture_brut)
		ymax=max(lecture_brut)
		self.trace_ax.set_xlim([xmin,xmax])
		...
		self.trace_line.set_label('sensi=%4.3E'%self.StR)
		...
		self.mu=statistics.mean(self.hist_data_moins)
		self.sigma=statistics.pstdev(self.hist_data_plus)
