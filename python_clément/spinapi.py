...
def pb_read_status(): #returns 1 if stopped and 6 if running (running+reset I guess)
	# Bit 0 - Stopped
	# Bit 1 - Reset
	# Bit 2 - Running
	# Bit 3 - Waiting 
	ret = spinapi.pb_read_status()
	return ret
...
if __name__=="__main__" :
	...
	#Inst.CONTINUE : passe à la ligne suivante; Inst.BRANCH, nomDeInst : retourne à nomDeInst; Inst.STOP : stop la séquence
	# Program the pulse program
	pb_start_programming(PULSE_PROGRAM)
	# start = pb_inst_pbonly(0b111111111111111111110001, Inst.CONTINUE, 0, 200.0 * ms)
	# pb_inst_pbonly(0x0, Inst.BRANCH, start, 200.0 * ms)
	print(0XFFFFF1)
	# start = pb_inst_pbonly(0, 0, 0, 2e8)
	pb_inst_pbonly(0XF00001, 6, 0, 2e8)
	pb_stop_programming()

	# Trigger the board
	print(pb_read_status())
	...
	pb_close()
	# print(pb_read_status()) #je crois qu'il faut pas le faire ça
