...
class Photon_Counter(QMainWindow):
	def __init__(self):
		...
		self.sampling_rate=int((self.n_points-1)/self.time_acq)
		...
		self.x=np.linspace(0,self.time_acq,self.n_points-1)
	...
	def start_measure(self):
		...
		self.digout.timing.cfg_samp_clk_timing(self.sampling_rate,source='/Dev1/Ctr1InternalOutput',sample_mode=nidaqmx.constants.AcquisitionType.CONTINUOUS, samps_per_chan=self.n_points)


		self.sample_clock.triggers.start_trigger.cfg_dig_edge_start_trig('/Dev1/do/StartTrigger')
 
		signal=[True]*(self.n_points//2)+[False]*1
		signal+=[True]*(self.n_points-len(signal))
		if len(signal) != self.n_points :
			print('problème coco')
