...
class Photon_Counter(QMainWindow):
	def __init__(self):
		...
		self.n_points=100
		self.V_AOM_list=np.linspace(0,0.5,self.n_points)
		self.n_per_point=10

		##Creation of the graphical interface##

		self.setWindowTitle("StR AoM")
	...
	def take_point(self):
		...
		if self.n_current_index==self.n_points-1:
			...
			V=self.V_AOM_list[self.n_current_index]
			with nidaqmx.Task() as tension_AOM :
				tension_AOM.ao_channels.add_ao_voltage_chan('Dev1/ao1')
				tension_AOM.write(V)
				tension_AOM.start()
			time.sleep(5)
