...
class Photon_Counter(QMainWindow):
	...
	def start_measure(self):
		...
		with open(self.fname,'w') as f:
			f.write('f_min=%f \t f_max=%f \t pos_min=%f \t pos_max=%f \n'%(self.f_min,self.f_max,self.pos_min,self.pos_max))
		self.list_pos=np.linspace(self.pos_max,self.pos_min,self.n_scan)
		self.index_pos=0
		self.start_ESR()
