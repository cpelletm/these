...
class Photon_Counter(QMainWindow):
	def __init__(self):
		...
		self.cbdownautoscale.setCheckState(True)
	...
	def update_canvas(self):       
		...
		self.y[-1]=(self.data[1]-self.data[0])/(self.dt*1e-3)
	...
	def start_measure(self):
		...
		self.task.timing.cfg_samp_clk_timing(1/self.dt,source='/Dev1/PFI9',sample_mode=nidaqmx.constants.AcquisitionType.CONTINUOUS, samps_per_chan=2)
		...
		#Adjust time axis
		self.t=np.arange(0,self.N*self.dt*1e-3,self.dt*1e-3)
		...
		def APD(dt):
			with open('PB_instructions.txt','w') as f:				
				f.write('label: 0b 0110, 20 ns \n') #en dessous de 20 ns il commence à déconner et à sauter des pulses
				f.write('\t0b 0100,  %f ms, branch, label'%dt) 

		APD(self.dt)
		check_output('spbicl load pb_instructions.txt 500.0') #500 C'est la seule fréquence ou les pulses ont la bonne valeur, il peut monter jusqu'à 1000 mais je vois pas l'interet
