...
class Photon_Counter(QMainWindow):
	...
	def start_measure(self):
		...
		def echo(taux,t_lect,t_ecl,pulse_pi_sur_2,pulse_pi):
			with open('PB_instructions.txt','w') as f:
				for i in range(len(taux)) :
					...
					f.write('\t0b 0100, '+pulse_pi_sur_2+'\n')
					f.write('\t0b 1100, '+tau+'\n')
					f.write('\t0b 0100, '+pulse_pi+'\n')
					f.write('\t0b 1100, '+tau+'\n')
					f.write('\t0b 0100, '+pulse_pi_sur_2+'\n')
		...
		echo(self.taux,self.t_lect,self.t_ecl,self.pulse_pi_sur_2,self.pulse_pi)
