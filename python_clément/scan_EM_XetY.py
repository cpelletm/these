...
class Photon_Counter(QMainWindow):
	def __init__(self):
		...
		self.dynamic_ax_X,self.dynamic_ax_Y= self.dynamic_canvas.figure.subplots(2)
	...
	def update_value(self):
		...
		self.dynamic_line_X.set_data(self.x[self.n_lect_min:self.n_lect_max],self.y_X[self.n_lect_min:self.n_lect_max])
		self.set_lim(x=self.x[self.n_lect_min:self.n_lect_max],y=self.y_X[self.n_lect_min:self.n_lect_max],ax=self.dynamic_ax_X,line=self.dynamic_line_X)

		self.y_Y=np.zeros(self.n_points)
		self.dynamic_line_Y.set_data(self.x[self.n_lect_min:self.n_lect_max],self.y_Y[self.n_lect_min:self.n_lect_max])
		self.set_lim(x=self.x[self.n_lect_min:self.n_lect_max],y=self.y_Y[self.n_lect_min:self.n_lect_max],ax=self.dynamic_ax_Y,line=self.dynamic_line_Y)
	...
	def update_canvas(self):       
		...
		lectureX=lecture[0]
		PLX=[sum(lectureX[i*self.n_glissant:(i+1)*self.n_glissant])/self.n_glissant for i in range(self.n_points//2,3*self.n_points//2)]
		PLX=np.array(PLX)
		...
		lectureY=lecture[1]
		PLY=[sum(lectureY[i*self.n_glissant:(i+1)*self.n_glissant])/self.n_glissant for i in range(self.n_points//2,3*self.n_points//2)]
		PLY=np.array(PLY)
		...
		if time.time()-self.time_last_refresh>self.refresh_rate :
			...
			if self.normalize_cb.isChecked() :
				...
				ytoplot=self.y_X
			self.dynamic_line_X.set_ydata(ytoplot[self.n_lect_min:self.n_lect_max])
			self.set_lim(x=self.x[self.n_lect_min:self.n_lect_max],y=ytoplot[self.n_lect_min:self.n_lect_max],ax=self.dynamic_ax_X,line=self.dynamic_line_X)

			if self.normalize_cb.isChecked() :
				ytoplot=self.y_Y/max(self.y_Y) 
			else :
				ytoplot=self.y_Y
			self.dynamic_line_Y.set_ydata(ytoplot[self.n_lect_min:self.n_lect_max])
			self.set_lim(x=self.x[self.n_lect_min:self.n_lect_max],y=ytoplot[self.n_lect_min:self.n_lect_max],ax=self.dynamic_ax_Y,line=self.dynamic_line_Y)
	...
	def start_measure(self):
		...
		self.tension.ai_channels.add_ai_voltage_chan("Dev1/ai5",min_val=-10,max_val=10)
