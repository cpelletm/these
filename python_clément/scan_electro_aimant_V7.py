...
class Photon_Counter(QMainWindow):
    def __init__(self):
        ...
        self.dynamic_ax,self.tension_plot_ax,self.bins_ax,self.PL_inst_ax = dynamic_canvas.figure.subplots(4)
        ...
        self.PL_inst_line,=self.PL_inst_ax.plot(self.x, self.y)
    ...
    def start_measure(self):
        ...
        self.PL_inst_line.set_data(self.t[1:],y_tension[1:])
        self.bins_ax.cla()
        ...
        self.PL_inst_ax.set_xlim([0,self.time_scan])
    ...
    def take_point(self):
        ...
        lecture=self.counter.read(self.N_scan)
        tensions=self.tension.read(self.N_scan)  
        
        self.PG.write('OUTP:GEN 0')
        self.PG.write('*WAI')
        time.sleep(0.1) #le temps que le générateur se remette à 0
        dt=1/self.sampling_rate
        nmin=round(0.1/2/dt)
        lecture=np.array(lecture)
        PL_tot=(lecture[1:]-lecture[:-1])*self.sampling_rate
        PL=(lecture[nmin:]-lecture[nmin-1:-1])*self.sampling_rate
        tensions_PL=tensions[nmin:]
        ...
        self.PL_inst_line.set_ydata(PL_tot) 
        ...
        self.PL_inst_ax.set_ylim([ymin*0.9,ymax*1.1]) 
