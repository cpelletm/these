...
class Photon_Counter(QMainWindow):
    ...
    def start_measure(self):
        ...
        self.Vmax_read=self.Vmax/3.9

        self.t=np.linspace(0,self.time_scan,self.N_scan)
        self.V=np.linspace(0,self.Vmax_read,self.N_display)
        ...
        marge=(self.Vmax_read-self.Vmin)*0.05
        self.tension_plot_ax.set_ylim([self.Vmin-marge,self.Vmax_read+marge])
        self.tension_plot_ax.set_xlim([0,self.time_scan])
        self.dynamic_ax.set_xlim([self.Vmin-marge,self.Vmax_read+marge])
    ...
    def take_point(self):
        ...
        for i in range(self.N_scan) :
            v=tensions[i]
            j=round((self.N_display-1)*v/self.Vmax_read)
            if j >= self.N_display :
