...
class Photon_Counter(QMainWindow):
    def __init__(self):
        ...
        self.x=np.linspace(self.Vmin,self.Vmax,self.N_scan)
    ...
    def start_measure(self):
        ...
        self.x=np.linspace(0,self.time_scan,self.N_scan)
        self.y=np.zeros(self.N_scan)
        self.dynamic_line.set_data(self.x, self.y)
        self.tension_plot_line.set_data(np.linspace(0,self.time_scan,self.N_scan+1), np.zeros(self.N_scan+1)) #Jtoujours les piquets et les intervalles
        self.dynamic_ax.set_xlim([0,self.time_scan])
    ...
    def take_point(self):
        ...
        tensions=self.tension.read(self.N_scan+1)
