...
tau_stretch=0.2
y=np.exp(-np.sqrt(x/tau_stretch))
plt.plot(x,y,label='stretch, tau=%f'%tau_stretch)

# y=np.exp(-x/tau_stretch)
# plt.plot(x,y,label='exp, tau=%f'%tau_stretch)

def lect(x,y,n_lect,t_up):
	y_out=[]
	xm=x[:n_lect]
	ym=np.exp(-np.sqrt(xm/t_up))*(0-1)+1
	ss=(sum(ym)-(ym[0]+ym[-1])/2)/(n_lect-1)

	for i in range(len(y)) :
		ym=np.exp(-np.sqrt(xm/t_up))*(y[i]-1)+1
		
		val=(sum(ym)-(ym[0]+ym[-1])/2)/(n_lect-1)-ss
		y_out+=[val]
	return(y_out)

y_lect=lect(x,y,10,0.1)

plt.plot(x,y_lect,label='lecture')

popt,yfit=stretch_exp_fit(x,y_lect)
plt.plot(x,yfit,label='fit stretch,tau=%f,ss=%f'%(popt[2],popt[1]))
