...
class Photon_Counter(QMainWindow):
    ...
    def start_measure(self):
        ...
        for i in range (1,self.twait+1) :
            self.y[i-1]=self.data[int(self.tecl*i+i*(i+1)/2+self.tread*i-1)]-self.data[int(self.tecl*i+i*(i+1)/2+self.tread*(i-1))]
        self.dynamic_line.set_ydata(self.y)
        ymax=max(self.y)
        self.dynamic_ax.set_ylim([0,ymax])        
        ...
        #Adjust time axis
        self.t=np.arange(0,len(self.y)*self.unit,self.unit)
...
app.show()
qapp.exec_()
app.counter.close() #just in case
