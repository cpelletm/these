...
\begin{align*}
\overline{R^2}&=\frac{1}{3} \int \rho(\omega_{NV}) \rho(\omega_{f}) \frac{4\gamma_f^2}{(\omega_f - \omega_{NV})^2+4\gamma_f^2} d\omega_{NV} d\omega_{f} \\
...
In the case where $\omega^0_{NV}=\omega^0_{f}$, meaning that either the considered NVs and fluctuators are from the same class, or from two classes perfectly resonant, by substituting $\overline{R ^2}$ for $(\bar{R})^2$ (which again is only approximately valid for $2 \gamma_f >> 1/T_2^*$) in eq. \ref{eq 1/T}, we find that:
\begin{equation}
...
Eq. \ref{eq 1/T avec T2*} is subject to many approximations: the inhomogeneous distribution of the NV centers is generally not Lorentzian, we don't know whether the fluctuator inhomogeneous broadening is the same than the NV one, and $\overline{R^2} \neq (\bar{R})^2$ in the general case. Nevertheless, eq. \ref{eq 1/T avec T2*} gives us a general idea of the evolution of $1/T$ with $T_2^*$.
