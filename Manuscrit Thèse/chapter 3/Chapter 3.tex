...
\subsection{$T_1$ dependency on magnetic field configuration}
...
It should be noted that the experimental values were heavily influenced by the choice of the parameter $T_1^{\rm ph}$. Using for example $T_1^{\rm ph}=2.5\ \rm ms$ gave results closer to the theoretical ones. Nevertheless, the fluctuator model generally tends to overestimate the depolarization rates. \citep{choi2017depolarization} for instance found that a 2-class degeneracy (the author did not precise whether this was a $\mathbf{B} \in \{110\}$ or $\mathbf{B} \in \{100\}$ configuration) yielded a depolarization rate $\sim 4$ times higher than that of a single isolated class, a value comparable to the one we found in table \ref{T1 champ mag}. We can also see in Fig. \ref{fluct linewidth} that a $\mathbf{B} \in \{110\}$ configuration lead to a $\sim 2.5$ times increase in $\Gamma_1^{\rm dd}$, compared to the predicted factor of 10.

This failure of the fluctuator model to accurately predict the changes in $T_1$ is one of the reasons that led us to believe that the model is incomplete.
...
I already mentioned that the discrepancy between the theoretical and experimental $T_1$  values were an issue of the current fluctuator model, although the discrepancy could come in part from the $T_1$ fitting procedure. Another, more glaring problem is the fact that some dipole-dipole mediated $T_1$ are not stretched exponential.

Fig. \ref{anormal T1} shows 
