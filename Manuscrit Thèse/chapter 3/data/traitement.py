...
def fit_exp_vs_stretch():
	...
	y=y/max(y)
	x=x*1e3
	plt.plot(x,y,'o',markerfacecolor='None',mew=0.8,ms=5,label='Experimental data')
	popt,yfit=exp_fit_zero(x,y)
	plt.plot(x,yfit,lw=2,label=r'exp(-$t/\tau$)')
	popt,yfit=stretch_exp_fit_zero(x,y)
	plt.plot(x,yfit,lw=2,label=r'exp(-$\sqrt{t/\tau}$)')
...
def fit_0B_vs_pas0B():
	...
	# popt,yfit=stretch_et_phonons(x,y,T1ph=5,fixed=False)
