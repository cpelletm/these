...
\section{Measuring the NV center spin properties}

One of the most important characterization of qubits is their relaxation times. Indeed, the potential applications of most qubits in quantum information or quantum sensing protocols is ultimately limited by their relaxation rates \citep{de2021materials, degen2017quantum}.

The relaxation times are generally decomposed between the longitudinal relaxation time $T_1$ corresponding to the relaxation time of the spin populations to their thermal equilibrium, and the transverse relaxation time $T_2$ (following the NMR nomenclature) corresponding to the relaxation of the spin state coherences (non diagonal elements of the density matrix).

The transverse relaxation time, also referred to as coherence time or dephasing time, is often further decomposed between the inhomogeneous dephasing time $T_2^*$ and the homogeneous dephasing time $T_2$. The $T_2^*$ time includes the inhomogeneous dephasing processes such as the in homogeneity in space (when multiple qubits are probed) and time of the external fields. The $T_2$ time corresponds to the coherence time where some of these inhomogeneities have been corrected, for instance with a Hanh echo sequence \citep{hahn1950spin} or a dynamical decoupling scheme \citep{naydenov2011dynamical}. While $T_2^*$ is an intrinsic property of the sample studied, $T_2$ depends on the rephasing scheme used. More careful nomenclatures will label the $T_2$ times $T_{2,\rm echo}$ or $T_{2,\rm DD}$ \citep{de2021materials}. The relaxation times are ordered as follows:
\begin{equation}
T_2^* < T_2 < 2 T_1.
\end{equation}

The relaxation times of NV centers depend in large part on their environment, and can therefore vary by several order of magnitude depending on the sample used. The following paragraphs will cover the measurement processes of the relaxation times and the typical values for the samples used in this manuscript.
...
The sequence shown in Fig.\ref{T1 basique} can be improved by implanting common mode rejection technique described in Fig.
