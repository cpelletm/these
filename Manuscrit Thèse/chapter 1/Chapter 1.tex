...
For samples in high excess of [N$_s$] such as type Ib HPHT diamonds, the question of the charge state of the NV center is of relatively low importance as the NV$^-$ state is the predominant one. For sample with a lower [N$_s$] content and a higher [NV]/[N$_s$] ratio however, one must be more careful with the optical intensity and wavelength used as the concentration of NV$^0$ can reach 50\% of the total NV concentration \citep{grezes2015storage}
...
Thanks to the discrete electronic transitions inside the diamond bad gap, the NV center is fluorescent: it can absorb light from $450-630$ nm and re-emits light in the $630-800$ nm band.
...
Fig. \ref{NV spin} illustrates the fine and hyper-fine structure of the NV center in zero external field.

Because $\gamma_n \ll \gamma_e$, the hyper-fine structure of the NV center can simply be thought of as a constant energy offset. This explains why for most intent and purposes the hyper-fine structure is ignored, along the electric and strain dependencies, which leads to the simpler spin Hamiltonian:

\begin{align}
\mathcal{H}&=D S_z^2 + \gamma_e \mathbf{S} \cdot \mathbf{B} \\
&=\begin{pmatrix}
D-\gamma_e B_z & \gamma_e (B_x+iB_y) & 0 \\
\gamma_e (B_x-iB_y) & 0 & \gamma_e (B_x+iB_y) \\
0 & \gamma_e (B_x+iB_y) & D+\gamma_e B_z
\end{pmatrix}
\end{align}
...
\medskip
The NV center's spin usage as a qubit is prevalent in quantum communication \citep{wehner2018quantum}, quantum computing \citep{de2021materials} and quantum sensing \citep{degen2017quantum}. Part of the reason is the overall long relaxation times (lifetime $T_1$ and coherence time $T_2$) of the order of a few ms at room temperature \citep{balasubramanian2009ultralong}, as well as the ability to address individual nuclear spins with even longer relaxation times \citep{awschalom2018quantum}. However the main advantage of the NV center spin compared to other spin qubit (quantum dots, superconducting) is its ability to be polarized and read-out optically thanks to the properties detailed in the following section.
...
The optical excitation and radiative decay between the $^3A_2$ and $^3E$ levels preserve the spin projection $m_s$ \citep{robledo2011spin}. There is however a non-radiative decay path which involves in inter-system crossing with the singlet states $^1A_1$ and $^1E$ which depends on the the triplet states spin projection.

Fig. \ref{ISC} summarizes the possible decay channels from the $^3E$ states. We assume that the electrons are initially excited from the $^3A_2$ states via a green laser with a rate $\gamma_las$. The $^3E$ states can then either decay radiatevely and emit a 637$-$800 nm photon, or undergo an intersystem crossing and decay through the singlet states. We do consider the decay channel through the singlet state as non-radiative due to the strong non radiative channels competing with the IR radiative transition (quantum efficiency $\leq 10^{-3}$ \citep{rogers2008infrared, ma2010excited, acosta2010optical}).

The rates $\gamma$ of these intersystem crossings, represented in Fig. \ref{ISC} by the time constant $\tau=1/\gamma$, strongly depends on the spin projection $m_s$: the likeliness to undergo a non-radiative decay is $\sim 4$\footnote{$\frac{15+91}{15+11}\approx 4$} times more likely when starting from the $\ket{\pm 1}$ states than when starting from the $\ket{0}$ state. Similarly, the singlet state is $\sim 2.4$\footnote{$\frac{492}{204} \approx 2.4$} more likely to decay in the $\ket{0}$ state than in the $\ket{\pm 1}$ states.

These spin selective intersystem crossings mean that under constant green excitation an optical pumping scheme in the $\ket{m_s=0}$ spin states takes place. At optical saturation, the polarization of the $\ket{0}$ state can reach $70-90\ \%$ \citep{gupta2016efficient}, which is equivalent to a thermal polarization (for B=0) with a spin temperature of $\approx 65\ \rm mK$ \footnote{\begin{equation*}
T=\frac{h D}{k_B \ln (\frac{1-P}{2P})} \approx 65\ \rm mK,
\end{equation*}
where $D$ is th ZFS factor described in eq. (\ref{eq. spin elec}) and $P\approx 0.8$ is the spin polarization defined as the population in the $\ket{0}$ state.} even for a crystal lattice temperature of 300 K. 

Another effect of the intersystem crossing is the ability optically readout the spin state of the NV center: since the $\ket{m_s=0}$ state is more likely to undergo a radiative decay than the $\ket{m_s=\pm 1}$ states, the expected photoluminescence (PL) is higher when starting from a $\ket{m_s=0}$ state than a $\ket{m_s=\pm 1}$ state. While the $\ket{+1}$ and $\ket{-1}$ cannot be a priori distinguished, a direct excitation of the $\ket{+1} \to \ket{0}$ or $\ket{-1} \to \ket{0}$ transition with a resonant microwave can ambiguously reveal the initial spin state.

Check le papier de vincent, ça m'a l'air tout faux ce truc. It should be noted that the decay rates indicated in Fig. \ref{ISC} cannot be directly measured. \citep{gupta2016efficient} infers them from short-time evolution of the PL after the laser is turned on. \citep{tetienne2012magnetic} used a different approach where they looked at the photodynamics as a function of the transverse magnetic field and found significantly different decay rates (by up to a factor of $\sim$ 2). 
