...
def lor_fit(x,y,Amp=None,x0=None,sigma=None,origin=0) :
	if not Amp :
		Amp=max(y)
	if not x0 :
		x0=x[int(len(x)/2)]
	...
	def f(x,Amp,x0,sigma,origin) :
		return Amp*1/(1+((x-x0)/(2*sigma))**2)+origin
...
def interp_linr() :
	...
	x=x[75:166]*44.4-78.5 #Les coefs sont extraits de conversion_V_NRJ (normalement b=-26.3 mais bon)
	y=y[75:166]
	droite=(y[90]-y[0])*(x-x[0])/(x[90]-x[0])+y[0]
	y=-y+droite
	plt.plot(x,y,'x',label='experimental data')
	Amp,x0,sigma,origin=lor_fit(x,y)
	def f(x,Amp,x0,sigma,origin) :
		return Amp*1/(1+((x-x0)/(2*sigma))**2)+origin
	plt.plot(x,f(x,Amp,x0,sigma,origin),label='HWHM=%f MHz'%(sigma*np.log(4)))
