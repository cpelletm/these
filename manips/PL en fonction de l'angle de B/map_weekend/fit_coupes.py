...
def gauss_fit(x,y,Amp=None,x0=None,sigma=None,origin=None) :
	if not Amp :
		Amp=max(y)
	if not x0 :
		x0=x[int(len(x)/2)]
	...
	def f(x,Amp,x0,sigma,origin) :
		return Amp*np.exp(-((x-x0)/(2*sigma))**2)+origin
...
x,y=extract_data('plot_1x2x1.txt')
x=x[20:80]
y=y[20:80]
plt.plot(x,y,'x',label='exp_data')
popt,yfit=lor_fit(x,y)
plt.plot(x,yfit,label='fit lor')
popt,yfit=gauss_fit(x,y)
plt.plot(x,yfit,label='fit gauss')
plt.legend()
plt.show()