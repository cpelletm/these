...
\
\title{Resonant Dipole-Dipole Induced Mechanical Rotation of a Diamond Crystal}
...
\begin{equation*}
  \hat{\mathcal{H}}_s=\hbar D S_z^2 + \hbar \gamma_e \textbf{B}\cdot\hat{\textbf{S}},
  \end{equation*} 
where $D = (2\pi) 2.87$ GHz is the crystal field splitting originating from spin-spin interactions, and $\gamma_e = 28 $GHz/T is the electron gyromagnetic ratio. We neglect contributions from the strain and local electric field in the hamiltonian since we are working with magnetic fields on the order of 100G. We also neglect the hyperfine interaction with the nuclear spin of the $^{14}$N atom since we are working with ensembles with a typical inhomogeneous broadening of 5 MHz. The \textbf{z} axis of the $S_z$ operator here is the axis formed by the nitrogen atom and the vacancy in the body frame.
