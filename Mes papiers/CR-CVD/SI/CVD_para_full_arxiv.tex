...
Then high energy (10MeV) electron irradiation at a fluence of $2\times18$ cm$^{-2}$ and at a temperature of 900 degrees celsius was realised, giving a final concentration of NV$^-$ of about 4.6~ppm for an initial concentration of nitrogen in the sample of about 25~ppm.
...
The downside of this choice of the [100] direction is that at large amplitudes, the transverse component of the magnetic field depolarizes all classes of NVs, thus limiting the magnetic field range that can be employed and therefore limiting this detection process to paramagnetic defects with zero-field splittings close to the NV center's one.
...
In order to let the three salient features detach better from the spectrum, we fitted a 4th-order polynomial to the PL evolution, without the spectral bumps, and subtract it from the data. We chose to use a 4th-order polynomial since numerical solving of the transverse-field dependent spin depolarization could not properly fit our results. The order of the polynomial fit was chosen to be 4 since higher order did not yield significantly better envelope fitting, and lower order were noticeably worse. Thanks to the subtraction, the width and contrast of each peak can now be compared and their position can be determined with greater accuracy. The second and third peaks were better fitted with Gaussian line shapes than Lorentzian ones, which is coherent with the fact that the inhomogeneous broadening of the various spin species dominate the coupling strength between the spins.
...
Because the sample we use has a natural 1.1\% abundance of $^{13}$C, we can actually use the first peak as a way to calibrate the concentration of VH$^-$ and WAR1 in our sample. Because there are three possible sites for a first-shell $^{13}$C-NV pair, we can estimate that the concentration of $^{13}$C-NV pair is about 3.3\% of the total concentration of NV centers, i.e. $\sim$135~ppb. We can then compare the area of the second and third peak to the first one to have a first order estimation of the concentration of VH$^-$ and WAR1. We find [VH$^-$]$\approx$195~ppb and [WAR1]$\approx$124~ppb. We decided to compare the area of the peaks instead of the amplitudes since the width of the peak is mostly due to inhomogeneous spread of the spins, and not to the spin-spin interaction strength. It should be noted that a severe drawback of this method is the loss of contrast for higher magnetic field amplitude, meaning in this case that the WAR1 concentration is probably undervalued.
...
\section*{Appendix}
\subsection{Photoluminescence as a function of magnetic field in a HPHT sample}
...
Notably, no other features could be detected at larger magnetic field values. We realized such scans on many different highly-doped HPHT samples and, systematically, no CR features are observed at 50 and 122 G. 
 ...
 \begin{figure}[htbp]
\centering
{\includegraphics[width=\linewidth]{sumi4_13C}}
...
\subsection{Magnetic field scan along the [111] direction}
\begin{figure}
